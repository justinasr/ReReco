        ...
        for i in range(self.connection_attempts):
            try:
                ...
                if response.status != 200:
                    self.logger.info("Problems (%d) with [%s] %s: %s" % (response.status, method, url, response.read()))
                    return None

                response_to_return = response.read()
                if not self.keep_open:
                    self.logger.debug('Closing connection for %s. Timeout %s' % (self.host_url, self.timeout))
