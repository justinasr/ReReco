...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {'dbName': 'requests', 'displayName': 'Requests', 'visible': 1},
  ...
  methods: {
    fetchObjects () {
      ...
      axios.get('api/search?db_name=chained_requests' + queryParams).then(response => {
    ...
    deleteChainedRequest: function(chainedRequest) {
      ...
      this.dialog.ok = function() {
        ...
        axios.delete('api/chained_requests/delete', {data: {'prepid': chainedRequest.prepid}}).then(() => {
          ...
          component.loading = false;
          component.clearDialog();
          component.showError("Error deleting chained request", error.response.data.message);
        });
      }
      this.dialog.cancel = function() {
        component.clearDialog();
